...
// Función para ver el tag en el frontend
const viewTag = async (tag) => {
  if (!tag.slug) {
    alert('Este tag no tiene un slug válido')
    return
  }
  ...
  try {
    ...
    const url = `https://${blogSubdomain}.${blogDomain}/tag/${tag.slug}`
    window.open(url, '_blank')
  } catch (error) {
    console.error('Error al obtener información del blog:', error)
    alert('Error al abrir el tag en el frontend')
  }
}
</script>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y border-panel">
        ...
        <tbody class="bg-panel divide-y border-panel">
          <tr v-for="tag in sortedTags" :key="tag.id" class="hover:bg-gray-50 transition-colors duration-150">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span class="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-700">{{ tag.postCount || 0 }}</span>
            </td>
