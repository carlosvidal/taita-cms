...
onMounted(async () => {
  ...
  try {
    ...
    if (!isEditMode.value && !post.value.categoryId) {
      const uncategorized = categories.value.find(c => c.slug === 'sin-categoria')
      if (uncategorized) {
        post.value.categoryId = uncategorized.id
        console.log('Categoría "Sin categoría" asignada por defecto:', uncategorized.id)
      }
    }
  } catch (error) {
